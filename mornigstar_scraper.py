...
def get_res_data(response, results):
    try:
        url = response.url
...
def run(playwright: Playwright, ticker):
    global res
    ...
    results = []
    page.on("response", lambda response: get_res_data(response,results))
...
if __name__ == "__main__":
    ...
    if os.path.exists(file):
        scraped_fund = pd.read_csv("Malaysia Fund Universe.csv", index_col=0)
    else:
